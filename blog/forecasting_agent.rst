...
.. post:: March 26, 2026
    ...
    :author: Flavia
    :description: Build a time series forecasting agent with Amazon Chronos and Amazon Bedrock AgentCore
    :keywords: Time Series Forecasting, Large Language Models, Amazon Chronos, Amazon Bedrock AgentCore
    :image_src: https://machine-learning-blog.s3.eu-west-2.amazonaws.com/forecasting_agent/architecture_diagram.png
    :image_alt: Architecture diagram showing a user communicating with a Strands agent hosted on Amazon Bedrock AgentCore Runtime, which uses Claude Sonnet 4.6 as the language model and invokes Chronos on Amazon Bedrock as a time series forecasting tool to generate probabilistic forecasts.
    :excerpt: 1
...
.. raw:: html
    ...
    <img
        ...
        style="width:100%"
        alt="Architecture diagram showing a user communicating with a Strands agent hosted on Amazon Bedrock AgentCore Runtime, which uses Claude Sonnet 4.6 as the language model and invokes Chronos on Amazon Bedrock as a time series forecasting tool to generate probabilistic forecasts."
    >

1. Overview
***************************************************************************************************************

In this post, we build a conversational time series forecasting agent using Chronos `[1] <#references>`_
on Amazon Bedrock and deploy it to Amazon Bedrock AgentCore. The agent exposes Chronos as a tool and handles multi-turn
conversations with short-term memory, allowing users to interactively generate forecasts and adjust
parameters across multiple turns.

2. Solution
***************************************************************************************************************

We start by deploying Chronos-Bolt `[2] <#references>`_ to a Bedrock endpoint hosted on a CPU EC2 instance using ``boto3``.
To use the code below, you need to provide the Bedrock Marketplace ARN of Chronos-Bolt in your AWS
region, the ARN of your Bedrock execution role, and a custom endpoint name. Remember to delete the
...
The ``bedrock_endpoint_arn`` returned by the code will be used for invocation inside the agent's tool.
It will also need to be included as the resource in a custom IAM policy that grants the AgentCore
execution role permission to invoke this endpoint.

2.1 Build the agent with Strands Agents
===============================================================================================================

To build the agent, we need three files: ``agent.py``, which contains the Strands agent, the forecasting tool definition
and the AgentCore app entrypoint, an empty ``__init__.py``, and a ``requirements.txt`` with the required dependencies.
...
``agent.py`` is shown below. It defines a `Strands agent <https://strandsagents.com/>`__ backed by Claude Sonnet 4.6 on Bedrock
which responds to forecasting requests using a ``generate_forecasts`` tool. The tool takes as input the historical time series values,
the prediction length and the quantile levels, and invokes the Chronos-Bolt endpoint on Bedrock to return the predicted mean and quantiles.
The agent is wrapped in a ``BedrockAgentCoreApp`` with an async streaming entrypoint that yields agent events - including text chunks and tool call information - back to the caller.

.. raw:: html

    <p>
    <code class="docutils literal notranslate">
    <span class="pre" style="font-weight:600">agent.py</span>
    </code>
    </p>
...
2.2 Deploy the agent to Amazon Bedrock AgentCore
===============================================================================================================

We deploy the agent using the AgentCore CLI.
...
.. important::
    ...
    Attach the following policy to the AgentCore execution role in IAM to allow the agent to invoke the Chronos-Bolt endpoint on Amazon Bedrock.
...
2.3 Test the agent in a Jupyter Notebook
===============================================================================================================

We start by importing the required libraries: ``boto3`` to invoke the
agent on Amazon Bedrock AgentCore, ``json`` to serialize the request
payload and deserialize the response, ``uuid`` to generate unique
session identifiers, and ``IPython.display`` to render the agent’s
responses as formatted ``Markdown`` in the notebook.
...
In the second turn, we provide a time series and ask the agent to
...
In the third turn, we provide the requested quantile levels and specify
that we want the mean instead of the median. The agent now has all the
information it needs and calls ``generate_forecasts``, returning the
forecast values and the 95% prediction interval.
...
.. raw:: html
    ...
    <table style="width: 60%; table-layout: fixed; text-align: center; font-size:90%;">
      <colgroup>
        <col style="width: 25%;">
        <col style="width: 25%;">
        <col style="width: 25%;">
